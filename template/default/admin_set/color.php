...
<table id="addeditable" border=1 cellpadding=0 cellspacing=0 bordercolor="#dddddd">
    <tr>
      ...
      <td>&nbsp;<input name="qianse" type="text" value="<?=$tpl_data['qianse']?$tpl_data['qianse']:'#f25f55'?>" id="qianse" style="width:60px" /> <?php foreach($qianse_arr as $v){?><span class="yanse qianse <?php if('#'.$v==$tpl_data['qianse']){?>cur<?php }?>" bg="#<?=$v?>" style="background:#<?=$v?>"></span><?php }?> <span class="zhishi">仅供参考，可自行设置</span></td>
    </tr>
    <tr>
      <td align="right">着重色：</td>
      <td>&nbsp;<input name="shense" type="text" value="<?=$tpl_data['shense']?$tpl_data['shense']:'#d83e35'?>" id="shense" style="width:60px" /> <?php foreach($shense_arr as $v){?><span class="yanse shense <?php if('#'.$v==$tpl_data['shense']){?>cur<?php }?>" bg="#<?=$v?>" style="background:#<?=$v?>"></span><?php }?> <span class="zhishi">仅供参考，可自行设置</span></td>
    </tr>
    <tr>
      <td align="right">&nbsp;</td>
