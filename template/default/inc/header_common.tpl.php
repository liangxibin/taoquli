<div class="top">
    <div class="top1000">
      <div class="topleft" style="display:none">
        <div class="topleftA">您好，欢迎来到淘趣利！  请
            <a href="<?=u('user','login')?>">
                <span class="c_fcolor">登录</span>
            </a> /
            <a href="<?=u('user','register')?>">
                <span class="c_fcolor">免费注册</span>
            </a> <?php if($app_show==1){?>或使用<?php }?>
        </div>
        <?php if($app_show==1){?>
        <div class=loginWays onmouseover=showLogin() onmouseout=showLogin()>
          <SPAN id=weibo_login class=firstWay>
            <A style="CURSOR: pointer" href="<?=u('api',$apps[0]['code'],array('do'=>'go'))?>">
                <img style="width:16px; height:16px" alt="用<?=$apps[0]['code']?>登录" src="<?=TPLURL?>/inc/images/login/<?=$apps[0]['code']?>_1.gif"><?=$apps[0]['title']?>登陆
            </A>
        ...
        <ul class=menuList>
          ...
          <li>
              <A href="<?=u('api','do',array('code'=>$row['code'],'do'=>'go'))?>">
                  <img style="width:16px; height:16px" alt='使用<?=$row['title']?>登录' src="<?=TPLURL?>/inc/images/login/<?=$row['code']?>_1.gif" />
                  <?=$row['title']?>帐户登录
              </A>
          </li>
          <?php }?>
...
function topHtml()
{
    /*<div class="topleftA" style="padding-top:10px;">
	<a href="<?=u('user')?>">
	<span class="c_fcolor">{$name}</span>
	</a>
...
$.ajax({
  ...
  <div class="topright"> 
    <ul>
      <li>
          <a href="javascript:;" onClick="AddFavorite('<?=SITEURL?>','<?=WEBNAME?>')">收藏本站</a>
      </li>
      <li>
          <a href="comm/shortcut.php" target="_blank">快捷桌面 </a>
      </li>
      <?php if($app_status==1){?>
      <li>
          <a href="<?=u('app','index')?>" target="_blank" style="*line-height:15px;">手机APP </a>
      </li>
      <?php }?>
      <?php if($webset['shangjia']['status']==1){?>
      <li>
          <a style="color:#F00" target="_blank" href="<?=u('hezuo','index')?>">商家中心</a>
      </li>
      <?php }?>
      <li id="fonta">
          <a class="c_fcolor" href="<?=u('help','index')?>" target="_blank">网站帮助</a>
      </li>
    </ul>
  </div>
</div>
...
<div class="logo">
  ...
  <a href="<?=SITEURL?>">
      <img src="<?=$dd_tpl_data['logo']?>" alt="<?=WEBNAME?>" style="height:70px; max-width:230px"/>
  </a>
</div>
    ...
    <div class='searchbox' id="searchbox">
        <div style="TEXT-AliGN: left;">
           <FORM style="FLOAT: left" class='frombox' method='get' name='formname' action='index.php' target="_blank" autocomplete="off" onsubmit="return checkSubFrom('#s-txt');">
               <input type="hidden" id="mod" name="mod" value="inc" class="mod" />
               <input type="hidden" id="act" name="act" value="check" class="act" />
...
?>
    <a class="fa-link" <?=$phone_url?> >
        <img src="<?=TPLURL?>/inc/images/right_sj_1.png" />
    </a>
</div>
</div>
</div>
...
<div class="c_bgcolor daohang" id="navdaohang">
  <div class="daohang1000">
	...
	?>
      <li class="c_bgcolor linav<?=$have_child_class?><?=$lastclass?>" <?=$dom_id?>>
          <a <?=$nav[$i]['target']?> class="anav c_fcolor" href="<?=$nav[$i]['link']?>"><?=$nav[$i]['title']?><?=$nav[$i]['type_img']?><?=$em?></a>
      <?php if($em!=''){?>
      <ul class="n-h-list c_hborder" style="background:#fff; border-style:solid; border-width:0px 1px;">
        <?php foreach($nav[$i]['child'] as $row){?>
        <li>
            <a class="c_border" <?=$row['target']?> href="<?=$row['link']?>"><?=$row['title']?> <?=$row['alt']?></a>
        </li>
        <?php }?>
	  </ul>
      <?php }?>
